    ...
    if len(message.command) == 2:
        ...
        if await db.remove_premium_access(user_id):
            ...
            await client.send_message(
                chat_id=user_id, text=script.PREMIUM_END_TEXT.format(user.mention)
            )
        else:
            await message.reply_text(
                "ᴜɴᴀʙʟᴇ ᴛᴏ ʀᴇᴍᴏᴠᴇ ᴜꜱᴇᴅ !\nᴀʀᴇ ʏᴏᴜ ꜱᴜʀᴇ, ɪᴛ ᴡᴀꜱ ᴀ ᴘʀᴇᴍɪᴜᴍ ᴜꜱᴇʀ ɪᴅ ?"
            )
    else:
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "🔥 ᴇxᴛᴇɴᴅ ᴘʟᴀɴ", callback_data="premium_info"
                            )
                        ]
                    ]
                ),
    ...
    try:
        ...
        if amount in STAR_PREMIUM_PLANS:
            try:
                ...
                await callback_query.answer(
                    "🚫 Error Processing Your Payment. Try again.", show_alert=True
                )
        else:
...
async def pre_checkout_handler(client, query: PreCheckoutQuery):
    try:
        if query.payload.startswith("dreamxpremium_"):
            ...
            await query.answer(
                success=False, error_message="⚠️ Invalid Purchase Type.", show_alert=True
            )
    except Exception as e:
