...
NAMED_REGEX = re.compile(
    r"Season\s*0*(\d{1,2})[\s\-,:]*Ep(?:isode)?\s*0*(\d{1,3})", re.IGNORECASE
)
EP_ONLY_RANGE = re.compile(
    r"\b(?:EP|Episode)0*(\d{1,3})\s*-\s*0*(\d{1,3})\b", re.IGNORECASE
)
...
def remove_ignored_words(text: str) -> str:
    ...
    return " ".join(
        word for word in text.split() if word.lower() not in IGNORE_WORDS_LOWER
    )
...
def schedule_update(bot, base_name, delay=5):
    ...
    pending_updates[base_name] = loop.call_later(
        delay, lambda: asyncio.create_task(update_movie_message(bot, base_name))
    )
...
async def media_handler(bot, message):
    ...
    media.file_type = next(
        ft for ft in ("document", "video", "audio") if hasattr(message, ft)
    )
    ...
    try:
        ...
        async with lock:
            await _process_with_lock(
                bot, filename, caption, media_info, base_name, processed
            )
...
async def _process_with_lock(bot, filename, caption, media_info, base_name, processed):
    ...
    if not movie_doc:
        ...
        movie_doc = {
            ...
            "imdb_url": (
                details.get("url", "") if not TMDB_POSTER else details.get("tmdb_url")
            ),
        ...
        try:
            ...
            if movie_doc:
                ...
                await db.movie_updates.update_one(
                    {"_id": base_name}, {"$push": {"files": file_data}}
                )
        ...
        try:
            ...
            try:
                await bot.delete_messages(
                    chat_id=MOVIE_UPDATE_CHANNEL, message_ids=message_id
                )
                await db.movie_updates.update_one(
                    {"_id": base_name},
...
def generate_movie_message(movie_doc, base_name):
    ...
    for file in movie_doc["files"]:
        if file["quality"] != "N/A":
            all_qualities.update(
                q.strip() for q in file["quality"].split(",") if q.strip()
            )
        if file["language"] != "N/A":
            all_languages.update(
                l.strip() for l in file["language"].split(",") if l.strip()
            )
        if file["ott_platform"] != "N/A":
            platforms = [
                p.strip() for p in file["ott_platform"].split("|") if p.strip()
            ]
    ...
    if episodes_by_season:
        ...
        for season, episodes in sorted(
            episodes_by_season.items(), key=lambda x: int(x[0])
        ):
            singles = []
            ranges = []
            ...
            all_ep_parts = collapsed + sorted(
                ranges, key=lambda s: int(s.split("-")[0])
            )
